...
def get_predicted_value(patient_symptoms):
    ...
    if all(value == 0 for value in input_vector):
        ...
        return "Unknown Disease", missing_symptoms
    predicted_disease = diseases_list[svc.predict([input_vector])[0]]
    return predicted_disease, missing_symptoms
@app.route("/")
def index():
...
def home():
    if request.method == 'POST':
        ...
        if symptoms == "Symptoms":
            ...
            predicted_disease, missing_symptoms = get_predicted_value(user_symptoms)
            if missing_symptoms:
                message = f"Missing symptoms: {', '.join(missing_symptoms)}"
                return render_template('index.html', message=message)
